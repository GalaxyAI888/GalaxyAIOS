...
import platform
import subprocess
from aistack.detectors.base import GPUDetector, SystemInfoDetector
...
from aistack.utils.platform import device_type_from_vendor
...
class Fastfetch(GPUDetector, SystemInfoDetector):
    def is_available(self) -> bool:
        logger.info("Checking if fastfetch is available...")
        try:
            self._run_command(self._command_version(), parse_output=False)
            logger.info("Fastfetch is available")
            return True
    ...
    def gather_gpu_info(self) -> GPUDevicesInfo:
        logger.info("Starting GPU info gathering with fastfetch...")
        command = self._command_gather_gpu()
        results = self._run_command(command)
        
        logger.info(f"Fastfetch returned {len(results)} result items")
        
        for i, result in enumerate(results):
            typ = result.get("type")
            r = result.get("result")
            
            logger.info(f"Processing result item {i}: type={typ}")

            if r is None:
                logger.warning(f"Result item {i} has no 'result' field")
                continue

            if typ == "GPU":
                logger.info(f"Found GPU data in result item {i}")
                gpu_devices = self._decode_gpu_devices(r)
                logger.info(f"Decoded {len(gpu_devices)} GPU devices")
                return gpu_devices
        
        logger.warning("No GPU data found in fastfetch results")
        return []

    def gather_system_info(self) -> SystemInfo:  # noqa: C901
        logger.info("Starting system info gathering with fastfetch...")
        command = self._command_gather_system()
        results = self._run_command(command)
        
        logger.info(f"Fastfetch returned {len(results)} result items for system info")
        ...
        for i, result in enumerate(results):
            ...
            logger.info(f"Processing system info item {i}: type={typ}")

            if r is None:
                logger.warning(f"System info item {i} has no 'result' field")
                continue

            if typ == "OS":
                logger.info(f"Processing OS info: {self._get_value(r, 'name')} {self._get_value(r, 'version')}")
                system_info.os = OperatingSystemInfo(
                    name=self._get_value(r, "name"),
                    version=self._get_value(r, "version"),
                )
            elif typ == "Kernel":
                logger.info(f"Processing Kernel info: {self._get_value(r, 'name')} {self._get_value(r, 'release')}")
                k = KernelInfo(
                    name=self._get_value(r, "name"),
        ...
        logger.info("System info gathering completed")
        return system_info

    def _decode_gpu_devices(self, result: str) -> GPUDevicesInfo:
        logger.info(f"Decoding GPU devices from {len(result)} raw GPU entries")
        devices = []
        ...
        for i, value in enumerate(list):
            ...
            if vender is None or vender == "":
                logger.warning(f"GPU entry {i} has no vendor, skipping")
                continue

            name = self._get_value(value, "name")
            index = self._get_value(value, "index")
            
            logger.info(f"Processing GPU {i}: {name} (vendor: {vender})")
            ...
            logger.info(f"Added GPU device: {name} (index: {index}, memory: {memory.total}MB, temp: {self._get_value(value, 'temperature')}°C)")
    ...
    def _run_command(self, command, parse_output=True):
        result = None
        logger.info(f"Executing fastfetch command: {' '.join(command)}")
        ...
        try:
            ...
            if result.stdout:
                ...
                if len(result.stdout) > 500:
                    logger.debug(f"Command stdout (truncated): {result.stdout[:500]}...")
            else:
                logger.info("Command stdout is empty")
                
            # 详细记录stderr
            if result.stderr:
                logger.info(f"Command stderr (full): {result.stderr}")
            else:
                logger.info("Command stderr is empty")
        ...
        if not parse_output:
            logger.info("Command completed, returning raw output")
            return output

        try:
            parsed_json = json.loads(output)
            logger.info("Command output parsed successfully as JSON")
            logger.info(f"Parsed JSON structure: {type(parsed_json)}")
            if isinstance(parsed_json, list):
                logger.info(f"Parsed JSON contains {len(parsed_json)} items")
            elif isinstance(parsed_json, dict):
                logger.info(f"Parsed JSON keys: {list(parsed_json.keys())}")
            return parsed_json
        except Exception as e:
            logger.error(f"Failed to parse command output as JSON: {e}")
            logger.error(f"Raw output (full): {output}")
            if len(output) > 500:
                logger.error(f"Raw output (truncated): {output[:500]}...")
            raise Exception(
    ...
    def _command_executable_path(self):
        ...
        with pkg_resources.path(
            "aistack.third_party.bin.fastfetch", command
        ) as executable_path:
    ...
    def _command_gather_gpu(self):
        with pkg_resources.path(
            "aistack.detectors.fastfetch", "config_gpu.jsonc"
        ) as config_path:
            executable_path = self._command_executable_path()
    ...
    def _command_gather_system(self):
        with pkg_resources.path(
            "aistack.detectors.fastfetch", "config_system_info.jsonc"
        ) as config_path:
            executable_path = self._command_executable_path()
            executable_command = [
                executable_path,
